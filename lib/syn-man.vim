...
syn case ignore
syn match  manReference       "\f\+([0-9][a-z]\=)"
...
"if getline(1) =~ '^[a-zA-Z_]\+([023])' 
if getline(1) =~ '^\f\+([01237]\+[a-z]\=).*' "by ishbguy
  syntax include @cCode <sfile>:p:h/c.vim
  syn match manCFuncDefinition  display "\<\h\w*\>\s*("me=e-1 contained
	"syn match  mymanReference       display "\f\+([0-9][a-z]\=)" contained
  "syn region manSynopsis start="^SYNOPSIS"hs=s+8 end="^\u\+\s*$"me=e-12 keepend contains=manSectionHeading,@cCode,manCFuncDefinition
  syn region manSynopsis start="^SYNOPSIS"hs=s+8 end="DESCRIPTION$"he=e keepend contains=manSectionHeading,@cCode,manCFuncDefinition
  syn region manDesc start="^\s\+\w\+"hs=s end="SEE ALSO$"he=e keepend contains=manSectionHeading,@cCode,manReference
endif
